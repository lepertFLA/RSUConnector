...
class CWin32Error  
{
    ...
    private:
        ...
        struct SMetaMem
        {
            ...
            int m_dwErrCode;   // Win32 error code
            int m_nMsgStrLen;           // i.e. _tcslen() + 1.
            
            char* getString() { return (char*) (this+1); }
        };

        void addRef( const CWin32Error& rOrig )
        {
            #ifdef __APPLE__
                __sync_fetch_and_add( &rOrig.metaMem()->m_nRefCnt, rOrig.metaMem()->m_nRefCnt + 1 );
            #elif _WIN32
        ...
        void releaseRef()
        {
            ...
            #ifdef __APPLE__
                __sync_fetch_and_sub( &metaMem()->m_nRefCnt, metaMem()->m_nRefCnt - 1 );
            #elif _WIN32
        ...
        void doFormatMessage( int dwLastErr )
        {
            m_szErrMsg = NULL;  
            char* pTemp = NULL;
            int nLen = 0;
        ...
        void allocCopyData( char* pTemp, int nLen, int dwLastErr )
        {
            SMetaMem* pSM = NULL;
            pSM = (SMetaMem*) new unsigned char [ sizeof(SMetaMem) + (nLen+1)*sizeof(char) ];
            if( ! pSM )
            {
                return;
            }
            ...
            ::memcpy( m_szErrMsg, pTemp, nLen*sizeof(char) );
            m_szErrMsg[ nLen ] = '\0';
        }
